...
class FileFinder:
                ...
                if any(file.endswith(ext) for ext in self.extensions):
                    ...
                    try:
                        with open(full_path, 'r', encoding='utf-8') as f:
                            file_content = f.read()
                            relative_path = full_path.replace(self.root_path, '')[1:]
                            self.all_file_contents.extend([
                                f'File Path: {relative_path}', file_content, '---'
                            ])
                            logger.info(f'Appended content from {relative_path}')
    ...
    def write_to_file(self):
        ...
        try:
            ...
            logger.info(f"Operation completed. All code written to {self.output_filename}")
        except Exception as e:
...
if __name__ == '__main__':
    ...
    extensions = {'.ts', '.tsx', '.js', '.jsx', '.css'}  # Add any other web development-related extensions as needed
